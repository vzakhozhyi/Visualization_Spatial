...
#results='hide', warning=FALSE, message=FALSE - to avoid showing all those hidden messages
...
plot(contriWA_geo, 
     ...
     col = "red", 
     cex = .6, # shrinking
     add=T) # VERY important should be TRUE - add on top of the original 
```
...
baseMap <- ms_dissolve(wazipMap) #dissolve - delete all the boarders inside the boarders
```
...
tm_shape(baseMap) + tm_polygons()+    #calling a layer and some components; file and then polygons from original file
    tm_shape(contriWA_geo) + tm_dots(size = 0.1,col = 'red',alpha=0.5) +    #calling file and then points from spatial data file; #ALPHA 0 is total transparency
    tm_layout(main.title = "Points",main.title.position = 'center') +       #like themes in ggplot
    tm_compass(position = c('left','TOP'),type = 'arrow')+                  #geography elements
    tm_scale_bar(position=c("RIGHT", "BOTTOM"),width = 0.2)+                #geography elements
    tm_credits(creditsText, position=c("left", "bottom"))                   #geography elements
